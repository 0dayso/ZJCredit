...
namespace System.Reflection
{
  ...
  internal class RuntimeAssembly : Assembly, ICustomQueryInterface
  {
    ...
    internal static RuntimeAssembly InternalLoadFrom(string assemblyFile, Evidence securityEvidence, byte[] hashValue, AssemblyHashAlgorithm hashAlgorithm, bool forIntrospection, bool suppressSecurityChecks, ref StackCrawlMark stackMark)
    {
      ...
      byte[] hash = hashValue;
      int num1 = (int) hashAlgorithm;
      assemblyRef.SetHashControl(hash, (AssemblyHashAlgorithm) num1);
      Evidence assemblySecurity = securityEvidence;
      ...
      int num2 = 1;
      int num3 = forIntrospection ? 1 : 0;
      int num4 = suppressSecurityChecks ? 1 : 0;
      return RuntimeAssembly.InternalLoadAssemblyName(assemblyRef, assemblySecurity, (RuntimeAssembly) local, stackMark1, num2 != 0, num3 != 0, num4 != 0);
    }
    ...
    internal static AssemblyName CreateAssemblyName(string assemblyString, bool forIntrospection, out RuntimeAssembly assemblyFromResolveEvent)
    {
      ...
      if (assemblyString.Length == 0 || (int) assemblyString[0] == 0)
      ...
      // ISSUE: explicit reference operation
      // ISSUE: variable of a reference type
      RuntimeAssembly& assembly = @assemblyFromResolveEvent;
      int num1 = forIntrospection ? 1 : 0;
      int num2 = 1;
      assemblyName.nInit(assembly, num1 != 0, num2 != 0);
      return assemblyName;
    }
    ...
    private static AssemblyName EnumerateCache(AssemblyName partialName)
    {
      ...
      string fullName = partialName.FullName;
      int num = 2;
      Fusion.ReadCache(alAssems, fullName, (uint) num);
    ...
    public override Module LoadModule(string moduleName, byte[] rawModule, byte[] rawSymbolStore)
    {
      ...
      byte[] rawModule1 = rawModule;
      int cbModule = rawModule1 != null ? rawModule.Length : 0;
      byte[] rawSymbolStore1 = rawSymbolStore;
      int cbSymbolStore = rawSymbolStore1 != null ? rawSymbolStore.Length : 0;
      ObjectHandleOnStack objectHandleOnStack = JitHelpers.GetObjectHandleOnStack<RuntimeModule>(ref o);
      RuntimeAssembly.LoadModule(nativeHandle, moduleName1, rawModule1, cbModule, rawSymbolStore1, cbSymbolStore, objectHandleOnStack);
      return (Module) o;
    }
    ...
    private static string VerifyCodeBase(string codebase)
    {
      ...
      if (num != -1 && num + 2 < length && ((int) codebase[num + 1] == 47 || (int) codebase[num + 1] == 92) && ((int) codebase[num + 2] == 47 || (int) codebase[num + 2] == 92))
        return codebase;
      if (length > 2 && (int) codebase[0] == 92 && (int) codebase[1] == 92)
    ...
    internal unsafe Stream GetManifestResourceStream(string name, ref StackCrawlMark stackMark, bool skipSecurityCheck)
    {
      ...
      if ((IntPtr) resource == IntPtr.Zero)
        return (Stream) null;
      if (length > 9223372036854775807UL)
    ...
    private static IPermission CreateWebPermission(string codeBase)
    {
      ...
      {
        object[] objArray = new object[2]{ (object) (Enum) Enum.Parse(type1, "Connect", true), null };
        if (objArray[0] != null)
        {
    ...
    private RuntimeModule OnModuleResolveEvent(string moduleName)
    {
      ...
      ModuleResolveEventHandler resolveEventHandler = this._ModuleResolve;
    ...
    private RuntimeAssembly InternalProbeForSatelliteAssemblyNextToParentAssembly(AssemblyName an, string name, string codeBase, CultureInfo culture, bool throwOnFileNotFound, bool useLoadFile, ref StackCrawlMark stackMark)
    {
      ...
      StringBuilder stringBuilder1 = new StringBuilder(useLoadFile ? str : codeBase, 0, useLoadFile ? str.LastIndexOf('\\') + 1 : codeBase.LastIndexOf('/') + 1, 260);
      stringBuilder1.Append(an.CultureInfo.Name);
      stringBuilder1.Append(useLoadFile ? '\\' : '/');
      ...
      try
      {
        try
        {
          runtimeAssembly = useLoadFile ? RuntimeAssembly.nLoadFile(string1, (Evidence) null) : RuntimeAssembly.nLoad(fileName, string1, (Evidence) null, this, ref stackMark, IntPtr.Zero, throwOnFileNotFound, false, false);
        }
        catch (FileNotFoundException ex)
        {
          notFoundException = new FileNotFoundException(string.Format((IFormatProvider) culture, Environment.GetResourceString("IO.FileNotFound_FileName"), (object) string1), string1);
          runtimeAssembly = (RuntimeAssembly) null;
        }
        if ((Assembly) runtimeAssembly == (Assembly) null)
        {
          StringBuilder stringBuilder2 = stringBuilder1;
          int startIndex = stringBuilder2.Length - 4;
          int length = 4;
          stringBuilder2.Remove(startIndex, length);
          stringBuilder1.Append(".EXE");
          string string2 = stringBuilder1.ToString();
          if (!useLoadFile)
            fileName.CodeBase = string2;
          try
          {
            runtimeAssembly = useLoadFile ? RuntimeAssembly.nLoadFile(string2, (Evidence) null) : RuntimeAssembly.nLoad(fileName, string2, (Evidence) null, this, ref stackMark, IntPtr.Zero, false, false, false);
          }
          catch (FileNotFoundException ex)
          {
            runtimeAssembly = (RuntimeAssembly) null;
    ...
    [SecurityCritical]
    [MethodImpl(MethodImplOptions.InternalCall)]
    internal static extern RuntimeAssembly nLoadImage(byte[] rawAssembly, byte[] rawSymbolStore, Evidence evidence, ref StackCrawlMark stackMark, bool fIntrospection, SecurityContextSource securityContextSource);
