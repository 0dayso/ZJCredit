...
namespace System.Diagnostics.Tracing
{
  ...
  {
    private static int[] nibblebits = new int[16]{ 0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4 };
    ...
    {
      while (idx < buffer.Length && (int) buffer[idx] != 0)
        ++idx;
      return idx;
    }
...
label_4:
    ...
    {
      return this.m_enabled && ((int) level <= (int) this.m_level || (int) this.m_level == 0) && (keywords == 0L || (keywords & this.m_anyKeywordMask) != 0L && (keywords & this.m_allKeywordMask) == this.m_allKeywordMask);
    }
                                          ...
                                          if (!(data is DateTime))
                                          {
                                            if (data is Enum)
                                            {
                                              ...
                                              else if (underlyingType == typeof (long))
                                              {
                                                ...
                                                // ISSUE: explicit reference operation
                                                // ISSUE: variable of a boxed type
                                                __Boxed<long> local2 = (ValueType) ((IConvertible) ^local1).ToInt64((IFormatProvider) null);
                                                // ISSUE: explicit reference operation
                                                ^local1 = (object) local2;
                                              }
                                              else
...
label_44:
    ...
    internal unsafe bool WriteEvent(ref EventDescriptor eventDescriptor, Guid* activityID, Guid* childActivityID, params object[] eventPayload)
    {
      ...
      if (this.IsEnabled(eventDescriptor.Level, eventDescriptor.Keywords))
      {
        ...
        byte* dataBuffer = stackalloc byte[32 * length1];
        bool flag = false;
        ...
        else
        {
          ...
          for (int index = 0; index < length2; ++index)
          {
            ...
            {
              fixed (byte* numPtr = (byte[]) objectList[index])
                eventDataPtr[intList[index]].Ptr = (ulong) numPtr;
            }
          }
          error = UnsafeNativeMethods.ManifestEtw.EventWriteTransferWrapper(this.m_regHandle, ref eventDescriptor, activityID, childActivityID, userDataCount, userData);
          for (int index = 0; index < length2; ++index)
    ...
    private unsafe uint EventRegister(ref Guid providerId, UnsafeNativeMethods.ManifestEtw.EtwEnableCallback enableCallback)
    {
      ...
      return UnsafeNativeMethods.ManifestEtw.EventRegister(ref providerId, enableCallback, (void*) null, out this.m_regHandle);
    }

    [SecurityCritical]
    private uint EventUnregister()
    {
      int num = (int) UnsafeNativeMethods.ManifestEtw.EventUnregister(this.m_regHandle);
      this.m_regHandle = 0L;
      return (uint) num;
    }
