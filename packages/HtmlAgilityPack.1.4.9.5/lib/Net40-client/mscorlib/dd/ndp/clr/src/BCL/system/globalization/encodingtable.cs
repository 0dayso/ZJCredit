...
namespace System.Globalization
{
  internal static class EncodingTable
  {
    ...
    private static unsafe int internalGetCodePageFromName(string name)
    {
      ...
      throw new ArgumentException(string.Format((IFormatProvider) CultureInfo.CurrentCulture, Environment.GetResourceString("Argument_EncodingNotSupported"), (object) name), "name");
    }
    ...
    internal static unsafe CodePageDataItem GetCodePageDataItem(int codepage)
    {
      ...
      int num;
      for (int dataIndex = 0; (num = (int) EncodingTable.codePageDataPtr[dataIndex].codePage) != 0; ++dataIndex)
      {
        if (num == codepage)
        {
