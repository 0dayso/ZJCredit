...
namespace System.Globalization
{
  ...
  internal class GregorianCalendarHelper
  {
      ...
      for (int index = 0; index < this.m_EraInfo.Length; ++index)
      {
        if (era == this.m_EraInfo[index].era)
        {
          if (year < this.m_EraInfo[index].minEraYear || year > this.m_EraInfo[index].maxEraYear)
            throw new ArgumentOutOfRangeException("year", string.Format((IFormatProvider) CultureInfo.CurrentCulture, Environment.GetResourceString("ArgumentOutOfRange_Range"), (object) this.m_EraInfo[index].minEraYear, (object) this.m_EraInfo[index].maxEraYear));
    ...
    internal virtual int GetDatePart(long ticks, int part)
    {
      ...
      if (part == 1)
        return num9 + 1;
      int[] numArray = (num8 != 3 ? 0 : (num6 != 24 ? 1 : (num4 == 3 ? 1 : 0))) != 0 ? GregorianCalendarHelper.DaysToMonth366 : GregorianCalendarHelper.DaysToMonth365;
      int index = num9 >> 6;
    ...
    public bool IsLeapDay(int year, int month, int day, int era)
    {
      if (day < 1 || day > this.GetDaysInMonth(year, month, era))
        throw new ArgumentOutOfRangeException("day", string.Format((IFormatProvider) CultureInfo.CurrentCulture, Environment.GetResourceString("ArgumentOutOfRange_Range"), (object) 1, (object) this.GetDaysInMonth(year, month, era)));
    ...
    public bool IsLeapMonth(int year, int month, int era)
    {
      year = this.GetGregorianYear(year, era);
      if (month < 1 || month > 12)
        throw new ArgumentOutOfRangeException("month", string.Format((IFormatProvider) CultureInfo.CurrentCulture, Environment.GetResourceString("ArgumentOutOfRange_Range"), (object) 1, (object) 12));
      return false;
    }
    ...
    public int ToFourDigitYear(int year, int twoDigitYearMax)
    {
      ...
      if (year < this.m_minYear || year > this.m_maxYear)
        throw new ArgumentOutOfRangeException("year", string.Format((IFormatProvider) CultureInfo.CurrentCulture, Environment.GetResourceString("ArgumentOutOfRange_Range"), (object) this.m_minYear, (object) this.m_maxYear));
      return year;
    }
  }
}
