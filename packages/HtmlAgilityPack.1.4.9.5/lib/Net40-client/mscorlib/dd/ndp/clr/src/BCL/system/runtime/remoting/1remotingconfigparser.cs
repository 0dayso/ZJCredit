...
namespace System.Runtime.Remoting.Activation
{
  internal static class RemotingXmlConfigFileParser
  {
    ...
    private static void ProcessLifetimeNode(ConfigNode parentNode, ConfigNode node, RemotingXmlConfigFileData configData)
    {
      ...
      foreach (DictionaryEntry attribute in node.Attributes)
      {
        string @string = attribute.Key.ToString();
        if (!(@string == "leaseTime"))
        {
          if (!(@string == "sponsorshipTimeout"))
          {
            if (!(@string == "renewOnCallTime"))
            {
              if (@string == "leaseManagerPollTime")
    ...
    private static RemotingXmlConfigFileData.ChannelEntry ProcessChannelsChannelNode(ConfigNode node, RemotingXmlConfigFileData configData, bool isTemplate)
    {
      ...
      foreach (DictionaryEntry attribute in node.Attributes)
      {
        string str2 = (string) attribute.Key;
        if (!(str2 == "displayName"))
        {
          if (!(str2 == "id"))
          {
            ...
            str1 = ((string) attribute.Value).ToLower(CultureInfo.InvariantCulture);
        }
      }
      if (typeName == null || assemName == null)
        RemotingXmlConfigFileParser.ReportMissingTypeAttributeError(node, "type", configData);
      ...
      RemotingXmlConfigFileParser._channelTemplates[(object) str1] = (object) channelEntry2;
    ...
    private static RemotingXmlConfigFileData.SinkProviderEntry ProcessSinkProviderNode(ConfigNode node, RemotingXmlConfigFileData configData, bool isTemplate, bool isServer)
    {
      ...
      if (isServer)
        RemotingXmlConfigFileParser._serverChannelSinkTemplates[(object) str1] = (object) sinkProviderEntry2;
      else
        RemotingXmlConfigFileParser._clientChannelSinkTemplates[(object) str1] = (object) sinkProviderEntry2;
      ...
      try
      {
        if (!char.IsDigit(c))
        {
          ...
          if (time.EndsWith("ms", StringComparison.Ordinal))
            num1 = 2;
          string str2 = time;
          int startIndex = str2.Length - num1;
          int length = num1;
          str1 = str2.Substring(startIndex, length);
        }
        int num2 = int.Parse(time.Substring(0, time.Length - num1), (IFormatProvider) CultureInfo.InvariantCulture);
        if (!(str1 == "d"))
        {
          if (!(str1 == "h"))
          {
