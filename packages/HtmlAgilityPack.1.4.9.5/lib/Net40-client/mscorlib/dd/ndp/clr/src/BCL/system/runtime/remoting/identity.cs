...
namespace System.Runtime.Remoting
{
  internal class Identity
  {
    ...
    {
      if (uri == null || uri.Length <= 1 || (int) uri[0] != 47)
        return uri;
      if (Identity.s_configuredAppDomainGuidString != null)
      {
        string prefix = Identity.s_configuredAppDomainGuidString;
        if (uri.Length > prefix.Length && Identity.StringStartsWith(uri, prefix))
          return uri.Substring(prefix.Length);
      }
      string prefix1 = Identity.s_originalAppDomainGuidString;
      if (uri.Length > prefix1.Length && Identity.StringStartsWith(uri, prefix1))
        return uri.Substring(prefix1.Length);
      string applicationName = RemotingConfiguration.ApplicationName;
      if (applicationName != null && uri.Length > applicationName.Length + 2 && (string.Compare(uri, 1, applicationName, 0, applicationName.Length, true, CultureInfo.InvariantCulture) == 0 && (int) uri[applicationName.Length + 1] == 47))
    ...
    internal void SetInIDTable()
    {
      ...
      do
      {
        comparand = this._flags;
        num = this._flags | 4;
      }
      while (comparand != Interlocked.CompareExchange(ref this._flags, num, comparand));
    }
    ...
    internal bool AddProxySideDynamicProperty(IDynamicProperty prop)
    {
      lock (this)
      {
        if (this._dph == null)
        {
          DynamicPropertyHolder local_2 = new DynamicPropertyHolder();
          lock (this)
          {
            if (this._dph == null)
              this._dph = local_2;
