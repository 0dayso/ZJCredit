  ...
  internal class GCHandleCookieTable
  {
    ...
    internal IntPtr FindOrAddHandle(IntPtr handle)
    {
      ...
      lock (this.m_syncObject)
      {
        ...
        if (this.m_FreeIndex < this.m_HandleList.Length && Volatile.Read(ref this.m_HandleList[this.m_FreeIndex]) == IntPtr.Zero)
        {
          ...
          this.m_FreeIndex = this.m_FreeIndex + 1;
        }
        else
        {
          for (this.m_FreeIndex = 0; this.m_FreeIndex < 16777215; this.m_FreeIndex = this.m_FreeIndex + 1)
          {
    ...
    internal void RemoveHandleIfPresent(IntPtr handle)
    {
      ...
      lock (this.m_syncObject)
      {
        if (!this.m_HandleToCookieMap.ContainsKey(handle))
          return;
        IntPtr local_2 = this.m_HandleToCookieMap[handle];
        if (!this.ValidateCookie(local_2))
          return;
        int local_3 = this.GetIndexFromCookie(local_2);
        ++this.m_CycleCounts[local_3];
        Volatile.Write(ref this.m_HandleList[local_3], IntPtr.Zero);
        this.m_HandleToCookieMap.Remove(handle);
        this.m_FreeIndex = local_3;
      }
    }
