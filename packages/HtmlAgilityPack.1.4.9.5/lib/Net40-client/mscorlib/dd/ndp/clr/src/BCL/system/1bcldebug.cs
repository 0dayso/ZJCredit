...
namespace System
{
  internal static class BCLDebug
  {
    ...
    private static void CheckRegistry()
    {
      ...
      try
      {
        int num = (int) BCLDebug.levelConversions[logLevel];
        ...
        for (int index = 0; index < BCLDebug.switches.Length; ++index)
        {
          if ((BCLDebug.switches[index].value & registryLoggingValues) != 0)
            new LogSwitch(BCLDebug.switches[index].name, BCLDebug.switches[index].name, Log.GlobalSwitch).MinimumLevel = (LoggingLevels) num;
        }
        Log.GlobalSwitch.MinimumLevel = (LoggingLevels) num;
    ...
    internal static bool CheckEnabled(string switchName)
    {
      ...
      if (@switch == null)
        return false;
      return @switch.MinimumLevel <= LoggingLevels.TraceLevel0;
    }
    ...
    internal static void ConsoleError(string msg)
    {
      ...
      if (BCLDebug.m_MakeConsoleErrorLoggingWork == null)
      {
        ...
        FileIOPermission fileIoPermission = new FileIOPermission(FileIOPermissionAccess.AllAccess, Path.GetFullPath("."));
