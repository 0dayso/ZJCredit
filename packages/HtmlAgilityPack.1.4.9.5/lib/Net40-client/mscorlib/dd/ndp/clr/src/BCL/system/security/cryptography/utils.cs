  ...
  internal static class Utils
  {
    ...
    internal static byte[] FixupKeyParity(byte[] key)
    {
      ...
      for (int index = 0; index < key.Length; ++index)
      {
        ...
        if ((int) (byte) ((int) num2 & 1 ^ (int) num2 >> 1) == 0)
    ...
    internal static byte[] RsaPkcs1Padding(RSA rsa, byte[] oid, byte[] hash)
    {
      ...
      int dstOffsetBytes = length1 - length2;
      if (dstOffsetBytes <= 2)
    ...
    internal static bool CompareBigIntArrays(byte[] lhs, byte[] rhs)
    {
      ...
      int index1 = 0;
      int index2 = 0;
      while (index1 < lhs.Length && (int) lhs[index1] == 0)
        ++index1;
      while (index2 < rhs.Length && (int) rhs[index2] == 0)
        ++index2;
    ...
    {
      byte[] o = (byte[]) null;
      SafeKeyHandle hKey1 = hKey;
      int keyNumber1 = keyNumber;
      int calgKey1 = calgKey;
      int calgHash1 = calgHash;
      byte[] hash1 = hash;
      int length = hash1.Length;
      ObjectHandleOnStack objectHandleOnStack = JitHelpers.GetObjectHandleOnStack<byte[]>(ref o);
      Utils.SignValue(hKey1, keyNumber1, calgKey1, calgHash1, hash1, length, objectHandleOnStack);
      return o;
    }
    ...
    internal static bool VerifySign(SafeKeyHandle hKey, int calgKey, int calgHash, byte[] hash, byte[] signature)
    {
      ...
      byte[] signature1 = signature;
      int length2 = signature1.Length;
      return Utils.VerifySign(hKey1, calgKey1, calgHash1, hash1, length1, signature1, length2);
    }
