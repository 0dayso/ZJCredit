  ...
  internal class StringExpressionSet
  {
    ...
    protected void GenerateString()
    {
      if (this.m_list != null)
      {
        ...
        while (enumerator.MoveNext())
        {
          ...
          if (str != null)
          {
            int num1 = str.IndexOf(StringExpressionSet.m_separators[0]);
            int num2 = -1;
            if (num1 != num2)
    ...
    protected virtual bool StringSubsetString(string left, string right, bool ignoreCase)
    {
      ...
      if (left.Length - right.Length == 1)
      {
        ...
        if ((int) str[index] == (int) StringExpressionSet.m_directorySeparator)
          return string.Compare(left, 0, right, 0, right.Length, comparisonType) == 0;
      }
      string str1 = right;
      int index1 = str1.Length - 1;
      if ((int) str1[index1] == (int) StringExpressionSet.m_directorySeparator || (int) left[right.Length] == (int) StringExpressionSet.m_directorySeparator)
    ...
    protected static bool StringSubsetStringPathDiscovery(string left, string right, bool ignoreCase)
    {
      ...
      if (string.Compare(strA, 0, strB, 0, strA.Length, comparisonType) != 0)
        return false;
      if (strA.Length == 3 && strA.EndsWith(":\\", StringComparison.Ordinal) && ((int) strA[0] >= 65 && (int) strA[0] <= 90 || (int) strA[0] >= 97 && (int) strA[0] <= 122))
        return true;
