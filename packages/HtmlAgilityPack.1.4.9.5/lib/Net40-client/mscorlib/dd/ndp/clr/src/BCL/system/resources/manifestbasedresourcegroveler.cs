  ...
  internal class ManifestBasedResourceGroveler : IResourceGroveler
  {
    ...
    public ResourceSet GrovelForResourceSet(CultureInfo culture, Dictionary<string, ResourceSet> localResourceSets, bool tryParents, bool createIfNotExists, ref StackCrawlMark stackMark)
    {
      ...
      else
      {
        ...
        if ((Assembly) satellite == (Assembly) null && (!culture.HasInvariantCultureName ? 0 : (this._mediator.FallbackLoc == UltimateResourceFallbackLocation.Satellite ? 1 : 0)) != 0)
    ...
    internal ResourceSet CreateResourceSet(Stream store, Assembly assembly)
    {
      if (store.CanSeek && store.Length > 4L)
      {
        ...
        if (binaryReader.ReadInt32() == ResourceManager.MagicNumber)
        {
          ...
          object[] args = new object[1]{ (object) (IResourceReader) Activator.CreateInstance(Type.GetType(str1, true), new object[1]{ (object) store }) };
      ...
      if (this._mediator.UserResourceSet == (Type) null)
        return (ResourceSet) new RuntimeResourceSet(store);
      object[] objArray = new object[2]{ (object) store, (object) assembly };
      try
      {
        try
        {
        ...
        return (ResourceSet) Activator.CreateInstance(this._mediator.UserResourceSet, new object[1]{ (object) store });
      }
      catch (MissingMethodException ex)
      {
    ...
    private void HandleSatelliteMissing()
    {
      ...
      byte[] publicKey = this._mediator.MainAssembly.GetPublicKey();
