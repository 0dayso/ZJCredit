  ...
  internal class ArraySortHelper<T> : IArraySortHelper<T>
  {
    ...
    private static IArraySortHelper<T> CreateArraySortHelper()
    {
      if (typeof (IComparable<T>).IsAssignableFrom(typeof (T)))
        ArraySortHelper<T>.defaultArraySortHelper = (IArraySortHelper<T>) RuntimeTypeHandle.Allocate(typeof (GenericArraySortHelper<string>).TypeHandle.Instantiate(new Type[1]{ typeof (T) }));
      else
