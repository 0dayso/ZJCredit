...
// Type: System.Reflection.Emit.__ExceptionInfo
...
namespace System.Reflection.Emit
{
  internal sealed class __ExceptionInfo
  {
    ...
    {
      this.m_startAddr = 0;
      this.m_filterAddr = (int[]) null;
      this.m_catchAddr = (int[]) null;
      this.m_catchEndAddr = (int[]) null;
      this.m_endAddr = 0;
      this.m_currentCatch = 0;
      this.m_type = (int[]) null;
      this.m_endFinally = -1;
    ...
    {
      Type[] typeArray = new Type[incoming.Length * 2];
      Array.Copy((Array) incoming, (Array) typeArray, incoming.Length);
      return typeArray;
    }
    ...
    private void MarkHelper(int catchorfilterAddr, int catchEndAddr, Type catchClass, int type)
    {
      ...
      else
      {
        ...
        this.m_currentCatch = this.m_currentCatch + 1;
      }
      if (this.m_endAddr != -1)
        return;
    ...
    internal void MarkCatchAddr(int catchAddr, Type catchException)
    {
      ...
      int num = catchAddr;
      Type catchClass = catchException;
      int type = 0;
      this.MarkHelper(num, num, catchClass, type);
    }
    ...
    internal void MarkFinallyAddr(int finallyAddr, int endCatchAddr)
    {
      ...
      this.MarkHelper(finallyAddr, endCatchAddr, (Type) null, 2);
    }
    ...
    {
      int index1 = exc.m_currentCatch - 1;
      int index2 = this.m_currentCatch - 1;
      return exc.m_catchEndAddr[index1] < this.m_catchEndAddr[index2] || exc.m_catchEndAddr[index1] == this.m_catchEndAddr[index2] && exc.GetEndAddress() > this.GetEndAddress();
    }
