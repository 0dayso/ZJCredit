...
namespace System.Runtime.Remoting.Messaging
{
  ...
  internal class Message : IMethodCallMessage, IMethodMessage, IMessage, IInternalMessage, ISerializable
  {
    ...
    public bool HasVarArgs
    {
      [SecurityCritical] get
      {
        if ((this._flags & 16) == 0 && (this._flags & 32) == 0)
          this._flags = this.InternalHasVarArgs() ? this._flags | 32 : this._flags | 16;
    ...
    public void GetAsyncBeginInfo(out AsyncCallback acbd, out object state);
    ...
    [SecurityCritical]
    [MethodImpl(MethodImplOptions.InternalCall)]
    private object InternalGetArg(int argNum);

    [SecurityCritical]
    [MethodImpl(MethodImplOptions.InternalCall)]
    private object[] InternalGetArgs();
