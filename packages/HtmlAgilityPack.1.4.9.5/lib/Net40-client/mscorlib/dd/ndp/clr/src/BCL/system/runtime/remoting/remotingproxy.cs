...
namespace System.Runtime.Remoting.Proxies
{
  ...
  internal class RemotingProxy : RealProxy, IRemotingTypeInfo
  {
    ...
    private static void Invoke(object NotUsed, ref MessageData msgData)
    {
      ...
      Delegate @delegate;
      if ((@delegate = message.GetThisPtr() as Delegate) == null)
      ...
      {
        remotingProxy.InternalInvoke((IMethodCallMessage) message, true, message.GetCallType());
      }
      else
      {
        ...
        ThreadPool.QueueUserWorkItem(new WaitCallback(AgileAsyncWorkerItem.ThreadPoolCallBack), (object) new AgileAsyncWorkerItem((IMethodCallMessage) message, (callType & 8) != 0 ? (AsyncResult) null : asyncResult, @delegate.Target));
        if ((callType & 8) != 0)
